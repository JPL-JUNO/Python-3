...
## [第 22 项：用数量可变的位置参数给函数涉及清晰的参数列表](../code/ch03/item22.py)

接受可变数量的位置参数(positional argument)可以使函数调用更清晰（这些位置参数通常简称为 varargs，或 star args，因为习惯用 *args 指代。）
...
当没有 `values` 时，必须传递一个空列表。最好完全省略第二个参数，在 Python 中通过在最后一个位置参数名称前添加 * 来完成此操作。`log` 的第一个参数是必需的，而任意数量的后续位置参数是可选的：
...
如果已经有一个序列（如列表）并且想要调用可变参数函数（如 log），可以使用 * 运算符来完成此操作。这会让 Python 将序列中的项作为位置参数传递给函数：
...
第一个问题是**这些可选位置参数在传递给函数之前始终会转换为元组**。这意味着如果函数的调用者在生成器上使用 * 运算符，它将被迭代直到耗尽。生成的元组包含生成器中的每个值，这可能会消耗大量内存并导致程序崩溃：
...
接受 `*args` 的函数最适合知道参数列表中的输入数量并且数量相当小的情况。`*args` 非常适合一起传递许多字面值或变量名称的函数调用。

`*args` 的第二个问题是，如果不修改每个函数调用，将来就无法向函数添加新的位置参数：
...
这种情况很难排查，程序不会报错，只是会采用错误的数据继续运行下去。为了彻底避免这种漏洞，在给这种 `*arg` 函数添加参数时，应该使用只能通过关键字来指定的参数。
> 要点
>
>1. 通过在 `def` 语句中使用 `*args`，函数可以接受可变数量的位置参数。
>2. 可以使用序列中的项作为带有 `*` 运算符的函数的位置参数。
>3. 将 `*` 运算符与生成器一起使用可能会导致程序内存不足并崩溃。
>4. 向接受 `*args` 的函数添加新的位置参数可能会引入难以检测的错误。
